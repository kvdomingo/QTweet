...
const asyncTimeout = <T = any>(f: Function, ms: number): Promise<T> =>
  new Promise(resolve =>
    setTimeout(() => {
      resolve(f());
    }, ms),
...
const handleDiscordPostError = async (error, qChannel: QChannel, type, msg, errorCount = 0): Promise<number> => {
  const errCode = error.statusCode || error.code || error.status;
  // We keep fucking up. Stop trying.
  if (errorCount >= 2) {
    log(`${errCode}: Discord servers failed receiving ${type} ${errorCount} times, giving up`, qChannel);
    return 3;
  }
  // New message type
  let newType = type;
  ...
  if (errCode === 404 || errCode === 10003 || (errCode === undefined && error.name === "TypeError")) {
    ...
    if (!obj) {
      ...
      log(error);
      logMsg = `${errCode || "no qChannel"}: This channel can't be built anymore, it should be deleted`;
    } else if (error.request && error.request.method === "GET") {
      logMsg = `${errCode}: Discord encountered an error getting ${error.request.path}`;
    } else {
      log(error);
      log(msg);
      logMsg = `${errCode} on channel`;
    }
  } else if (errCode === 403 || errCode === 50013 || errCode === 50001 || errCode === 50004 || errCode === 40001) {
    retCode = 2;
    // Discord MissingPermissions error
    // Try to notify the user that something is wrong
    logMsg = `Tried to post ${type} but lacked permissions: ${errCode} ${error.name}`;
    ...
    (!Number.isNaN(errCode) && Number(errCode) >= 500 && Number(errCode) < 600) ||
    errCode === "ECONNRESET" ||
    ...
    logMsg = `Posting ${type} failed (${errCode} ${error.name}): ${error.message}`;
    log(msg);
    channelToPostIn = "none";
  }
  ...
  return asyncTimeout<number>(async (): Promise<number> => {
    try {
      ...
      return handleDiscordPostError(err, targetChannel, newType, newMsg, errorCount + 1);
    }
    return retCode;
  }, delay);
};
...
export const translated = async (qChannel: QChannel, key: string, options: i18nOptions = {}) =>
