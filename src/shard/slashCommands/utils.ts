...
export const createSlashCommand = (def: SlashCommandDefinition) => {
  ...
  def.options.forEach(({ type, name, description, required }) => {
    if (type === "number") {
      b.addNumberOption(opt => opt.setName(name).setDescription(description).setRequired(!!required));
    } else if (type === "string") {
      b.addStringOption(opt => opt.setName(name).setDescription(description).setRequired(!!required));
    } else {
      b.addBooleanOption(opt => opt.setName(name).setDescription(description).setRequired(!!required));
    }
  });
  return b;
};
