...
export const removeSubscription = async (channelId: string, twitterId: string) => {
  const { rowCount } = await pool.query('DELETE FROM subs WHERE "channelId" = $1 AND "twitterId" = $2', [
    channelId,
    twitterId,
  ]);
  return rowCount;
...
export const getChannelSubs = async (channelId: string, withName = false) => {
  const { rows } = await pool.query<DbSubscription>(
    withName
      ? `SELECT ${getInt('subs."twitterId"', '"twitterId"')},
    "name",
    "flags",
    "msg" 
    FROM subs INNER JOIN twitterUsers ON subs."twitterId" = twitterUsers."twitterId"
    WHERE subs."channelId"=$1`
      : `SELECT ${getInt('"twitterId"')}, "flags" FROM subs WHERE subs."channelId"=$1`,
    [channelId],
  );
  return rows;
};
...
export const getUserSubs = async (twitterId: string, withInfo = false) => {
  const { rows } = await pool.query<DbSubscription>(
    withInfo
      ? `SELECT ${getInt('subs."channelId"', '"channelId"')}, "flags", ${getInt('"guildId"')}, ${getInt(
          '"ownerId"',
        )}, "subs."isDM"" AS "isDM" FROM subs INNER JOIN channels ON subs."channelId" = channels."channelId" WHERE subs."twitterId"=$1;`
      : `SELECT ${getInt('"channelId"')}, "flags", "isDM", "msg" FROM subs WHERE "twitterId"=$1`,
    [twitterId],
  );
  return rows;
};
...
export const add = async (
  { channelId, isDM, guildId, ownerId }: QCSerialized & { guildId: string; ownerId: string },
  twitterId: string,
  name: string,
  flags: number,
  msg: string,
