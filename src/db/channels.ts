...
export const getChannel = async (channelId: string): Promise<DbChannel | undefined> => {
  const {
    rows: [channel],
  } = await pool.query('SELECT * FROM channels WHERE "channelId" = $1', [channelId]);
  return channel;
};

const addChannelSQL = async (channelId: string, guildId: string, ownerId: string, isDM: boolean) => {
  const { rowCount } = await pool.query(
    'INSERT INTO channels("channelId", "guildId", "ownerId", "isDM") VALUES($1, $2, $3, $4) ON CONFLICT DO NOTHING',
...
export const getUniqueChannels = async (): Promise<DbChannel[]> => {
  const { rows } = await pool.query(
    `SELECT ${getInt('MIN("channelId")', '"channelId"')}, BOOL_OR("isDM") AS "isDM" FROM channels GROUP BY "guildId"`,
  );
  return rows;
};
