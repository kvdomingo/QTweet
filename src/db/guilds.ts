...
export const createGuild = async (guildId: string) => {
  const { rowCount } = await pool.query('INSERT INTO guilds("guildId") VALUES($1) ON CONFLICT DO NOTHING', [guildId]);
  return rowCount;
};
...
const getLangSQL = async (guildId: string) => {
  const {
    rows: [lang],
  } = await pool.query<{ lang: string }>('SELECT "lang" FROM guilds WHERE "guildId"=$1', [guildId]);
  return lang;
};
...
const getGuildInfoSQL = async (guildId: string): Promise<DbGuild | undefined> => {
  const {
    rows: [data],
  } = await pool.query<DbGuild>('SELECT * FROM guilds WHERE "guildId"=$1 LIMIT 1', [guildId]);
  return data;
};
