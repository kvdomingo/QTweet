...
const shardReady = async () => {
  if (mgr().shards.size !== mgr().totalShards || !mgr().shards.every(shard => shard.ready)) return;
  log("✅ All shards are ready!");
  // All shards are ready, start taking messages
  mgr().shards.each(shard =>
...
const start = async () => {
  ...
  try {
    log("Spawning shards...");
    manager.spawn({
      delay: Number(process.env.SHARD_SPAWN_DELAY || 15000),
      timeout: Number(process.env.SHARD_SPAWN_TIMEOUT || 60000),
    });
  } catch (e) {
